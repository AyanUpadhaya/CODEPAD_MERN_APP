...
function Search({ divClass, ...rest }) {
  const [isFocused, setIsFocused] = useState(false);
  return (
    <div
      className={`hide-outline flex items-center gap-2 border border-gray-200 px-4 py-3 rounded-full ${divClass} h-[48px]`}
    >
      <svg
        width="24"
        height="24"
      ...
      <input
        type="text"
        className="form-input outline-none focus:outline-none w-full rounded-none bg-transparent p-0 text-black-900 "
        autoComplete="false"
        {...rest}
      />
    </div>
